...
| **Translate: a line break inside a passage silently truncates the translation** | Found 2026-09-18 while building long-prose samples for the engine comparison. LFM2.5-1.2B-Instruct ends its turn at the first line break in the passage: the response is a translation of only the text before the break and `finish_reason` is `stop` (not `length`), so no layer reports the loss and the reader plays a partial passage. Reproduced on the S22-arm64-derived host build and on the device form of the same prompt: 3 of 6 single-paragraph probes with a `\n` inserted after the first sentence (132→21, 161→34, 110→33 generated tokens, 3 continued whole), and every multi-paragraph block (73 tokens against 235 for the same text with the breaks replaced by spaces). Exposure is the TXT/Markdown import path: `TextParser.buildChapters` joins wrapped source lines with `'\n'` (`TextParser.kt:143`) and flushes `TextPassage(paragraph.toString().trim())`, which keeps the internal breaks, while `LlamaTranslator.userMessage` does no whitespace normalization. EPUB and MOBI are immune — `OpfBookReader.extractParagraphs` collapses every whitespace run to a single space (`OpfBookReader.kt:221`) before splitting. | Open — recorded, not fixed: one normalization at the prompt builder (or collapsing whitespace at the passage grain) closes it; the owner has not ruled on where it belongs | prose-sample pass 2026-09-18; decisions #181; `docs/prints/book-samples/samples.md` §"Defect probe" |
