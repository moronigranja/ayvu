...
| **Rotation (any Activity recreation) starts playback at the share target without a press** | Owner report 2026-09-22: *"when changing orientation, playback starts without pressing play."* Traced path — the only auto-play in the code that does not come from a press: a reader opened from the share/identify "Listen here" target (S3) is given `startAt` from `MainActivity`'s **`rememberSaveable`** `targetChapter`/`targetPassage` (cleared only by close/settings), and `ReaderScreen`'s "starts audio at the passage **once**" effect is a `LaunchedEffect(startAt)` — one-shot per *composition*. `MainActivity` declares no `android:configChanges`, so rotation recreates the Activity; the saved target re-enters the reader, the effect re-runs, and `playPosition` fires `ACTION_PLAY_POSITION`: audio starts at the shared passage (and the reading place is pushed into the undo ring) with no press. `MainActivity.onCreate` also re-consumes the same share intent on the recreate, re-arming the target. Scope: a plain (non-share) reader open keeps `targetChapter = -1` → `startAt = null` → no auto-play, so if this was hit without a share-target open the trace is incomplete and needs re-scoping. | Open — recorded, not fixed (device reproduction pending; the `am start` shortcut needs a library book id) | owner report 2026-09-22; code trace `ReaderScreen.kt:205-209` + `MainActivity.kt:87-118` + manifest (no `configChanges`) |
