...
| Translate-then-read (`core-translate`) | **LANDED 2026-09-14, decisions #160; ENGINE SWAPPED 2026-09-15, decisions #162** — implemented end-to-end and DEVICE-VERIFIED on the S22 under SMaLL-100 (pt-BR playback under the auto-picked voice at 121–1352 ms per passage, `x<lang>` cache separation incl. the Off toggle, offline pregen under translation, idle-close logcat-verified; three defects found and fixed: firstVoiceFor case mismatch, target-voice pack-readiness gate, render-truth cache keys). The translator is now **LFM2.5-1.2B-Instruct on llama.cpp** (`:core-llm`, #162): chrF 67.37 vs 62.77, ~730 MB pack, `pregen` keys carry a `t<translator>` segment, SMaLL-100 deleted with no fallback. **DEVICE-VERIFIED 2026-09-15, instrumentation + UI pass** (`LfmTranslateE2eTest`, `LfmTranslatedPlaybackE2eTest`, then the S22's own screens): sha-verified download through the Read-in dialog, Speech subscreen showing "LFM2.5-1.2B translate model — ready · installed", pt-BR playback on a real book at 2.2–4.1 s/passage under concurrent Kokoro synthesis, renders cached under `xpt-BR/tlfm12b/` (pre-#162 small-100 audio correctly not a hit), retired small-100 artifacts reclaimed, zero lmkd kills. Remaining: a full whole-book pre-gen run under the new translator (~2 h on Jumper) and the HiBreak co-residency check (not attached). **Engine choice re-measured 2026-09-18 (decisions #181):** translategemma-4b is cleanly better on real prose and was **rejected on cost** — 9.98 s/sentence idle and 20.8 s under Kokoro playback vs LFM's 3.30/4.42, 2.93 GB resident vs 1.54 GB, with lmkd reclaiming a third-party process under load; the FLORES gate still ranks LFM higher (67.37 vs 66.04) and the middle tier **LFM2.5-2.6B-Base was measured on 2026-09-20**: chrF 70.44 on-device both idle and during playback, 4.95/7.79 s per sentence, 1.67 GB pack, zero lmkd events — **Adopted as the second engine option (#182; its pack release is still to be published):** the default is unchanged (LFM2.5-1.2B) and the 2.6B model is user-selectable in Settings → Speech, each engine's pack downloaded and staged separately and every translation keyed by (language, engine id), so switching engines re-translates instead of reusing the other engine's renders (the old ones stay on disk). One finding was mis-recorded as a shipped defect and is corrected: the line-break truncation was measured on a host Q4_0 copy, and the shipped Q4_K_M does not reproduce it at the app's passage grain (`docs/prints/book-samples/break_truncation_probe.py`); the prompt builder normalizes whitespace anyway (#185). |
...
| Previous-passage context in the translate prompt | **MEASURED, NOT BUILT (2026-09-20, decisions #188).** Owner question: prepend the previous passage's translation (reading order) to the prompt. Pinned-artifact A/B (1.2B-Instruct + 2.6B-Base, greedy, `cache_prompt: false`) over 40 FLORES sentences + 40 consecutive real prose paragraphs, 6 prompt shapes. The shape that says exactly the ask — the previous passage's translation ALONE as the assistant turn — is statistically INERT on both corpora and both engines (book BT-chrF Δ −1.14 [−6.10, 3.68] on the shipped default, +3.14 [−3.94, 10.74] on the 2.6B; FLORES Δ inside ±2.9) while costing ~1.9× prefill (63.6→119 tokens/passage ≈ +0.6 s on the device's prefill rate). Carrying the previous SOURCE or label blocks moves quality a lot and contradicts itself across engines (D: rarely −5.4 on the default, +2.8 on the 2.6B). Reading order also makes the translation recursive — passage *i* needs *i*−1's context-conditioned text — so a cached row cannot be reproduced without replaying the chain (#161/#166 key the store on `(lang, translator)`). Reopens with a key-level context dimension, a shape that helps both engines, and a human blind read. Print: `docs/prints/context-spike/`. |
