...
| Previous-paragraph context in the translate prompt | **MEASURED, NOT BUILT (2026-09-20, decisions #188).** Owner question: prepend the last translated paragraph to the prompt. Pinned-artifact A/B (1.2B-Instruct + 2.6B-Base, greedy, `cache_prompt: false`) on 40 FLORES devtest sentences and 40 consecutive real prose paragraphs, 5 prompt shapes: on the SHIPPED DEFAULT the context reliably hurts (paired-bootstrap BT-chrF A−D = +5.44, 95% CI [0.97, 10.78]; every single-message shape much worse, the fenced one broken with 38/40 outputs re-emitting the context); on the 2.6B-Base it is a noise-floor gain (A−D = −2.82, CI [−6.15, −0.25]) costing 2.8× prefill tokens (63→173 per passage; at the measured ~100 tok/s device prefill ≈ +1.1 s/passage). Independent of quality it breaks the stored-translation identity: "the last translated paragraph" is a property of translation ORDER, not of `(bookId, chapter, passageIndex, lang, translator)` (#161/#166), so a prefetch/seek/pregen order would change a cached translation. Reopens only with a context dimension on the key AND a human blind read (the proxy is coarse). Print: `docs/prints/context-spike/`. |
