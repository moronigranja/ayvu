...
| Previous-passage context in the translate prompt | **MEASURED — a candidate on the 2.6B, not built (2026-09-20, decisions #188).** Owner: put the previous passage (reading order) in the prompt. Pinned-artifact A/B, 6 shapes, 2 books + FLORES, paired bootstraps: on the **2.6B-Base** the previous passage's source + its translation as a chat turn pair is a small consistent win (nonfiction BT-chrF 71.06→73.57, Δ−2.82 [−6.15, −0.25]; fiction 69.09→70.16, n.s.); on the **shipped default** the same shape reliably loses (64.43→58.16, P 0.994). **Depth 1 is the optimum** — 2/4/8 add nothing (all inside noise) at 3–14× the prefill. The window is a **hard ceiling of ~15–17 passages** (110–122 tokens per pair, n_ctx 2048; over it the runtime returns HTTP 400, not a silent trim). Shipping needs a context dimension + depth cap on the translation key and a chain rule for out-of-order translation (#161/#166). Reopen on the owner's blind read of `docs/prints/context-spike/samples.md` / `samples-book2.md`. |
