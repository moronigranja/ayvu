...
| Previous-passage context in the translate prompt | **ABANDONED (owner, 2026-09-20 — decisions #188).** Owner: put the previous passage (reading order) in the prompt; measured on both shipped engines, 6 shapes, 2 books + FLORES, paired bootstraps; then dropped because the gain is too small for the cost. The winning shape (previous source + its translation as a chat turn pair) helps only the **2.6B-Base** and only a little (nonfiction BT-chrF 71.06→73.57, Δ−2.82 [−6.15, −0.25]; fiction 69.09→70.16, n.s.) while the SAME shape reliably loses on the shipped default (64.43→58.16, P 0.994); it costs 1.9–2.8× the prefill at depth 1 and every deeper window is inside noise at 3–14× the prefill (the window caps hard at ~15–17 passages — `n_ctx` 2048, then HTTP 400, never a silent trim). No context dimension goes on the translation key. Record + samples: `docs/prints/context-spike/`. |
