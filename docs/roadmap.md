...
owner): two dominant classes confirmed and recorded — `abbrev-not-expanded`
...
`abbrev-period-pause`, and `date-slash-read-aloud` (slash-dates spoken as numbers
with the slash verbalized). ja/cmn/hi are recorded as a non-native-ear limitation.
