...
- **Engine tiers** (all behind `TTSEngine`; select per measured need):
  ...
  | **Piper** (small tier — D4, adopted 2026-09-13 #154) | VITS, 14–100 MB per-voice ONNX | Mostly flat | ~20+ langs incl. de/ko (Kokoro's gaps) | MIT. Direct-ORT port (not sherpa) keeps espeak-ng/JNA; per-voice model+config packs via TtsPack (pins: lessac en-US, thorsten de-DE, davefx es-ES, serena it-IT, faber pt-BR @ `1162a917`; `ko_KR-kss` gated on CC-BY-NC — #159); the stock export exposes no alignments → passage-level read-along only (#30b); quality gate passed (owner listening pass, #99 addendum). |
