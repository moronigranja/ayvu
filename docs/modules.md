...
core-translate/   LIVE — read-in-language seam (#114/#160/#162): the TranslatingEngine TTSEngine decorator (degrade-to-original on any failure), the language surface (LfmLang prompt names + TranslateLanguages catalog matching), the translate pack descriptor + TranslatePackStager; model-agnostic by construction (a suspend translate lambda)
core-llm/         LIVE — the translate runtime's native leg (#162): llama.cpp vendored @ b75ecd1971 into the gitignored build/llama.cpp-src (tools/fetch-llama-cpp.sh) + AGP externalNativeBuild (GGML_CPU_ALL_VARIANTS/GGML_BACKEND_DL, arm64-v8a only) + LlamaTranslator (JNI session, single-flight, greedy)
...
feature-settings/ LIVE — V1: settings screen (root + Speech subscreen — pane pattern shared with the OCR-languages picker, #156/#48), packs download UI (engine-agnostic, descriptor-derived — K2 #156), voice picker + favorites, threshold, OCR langs, theme; AndroidHttpTransport; Offline-audio section (per-book usage + delete, #44); "Backup & restore" SAF export/import (E1, #111); About group (version via the AppInfo seam, GPL-3.0 source link, NOTICE link, on-device privacy line — release 0.1.1)
