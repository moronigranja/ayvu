...
class RoomPlayerStoreTest {
    ...
    @Test
    fun `progress round-trips with offset and speed`() =
        runTest {
    ...
    @Test
    fun `commit pushes the ring newest-first and writes the row atomically`() =
        runTest {
    ...
    @Test
    fun `no ring push means the ring is untouched`() =
        runTest {
    ...
    @Test
    fun `pop consumes the newest entry`() =
        runTest {
            store.commitProgress(progress(0, 0), position(0, 0))
            store.commitProgress(progress(0, 1), position(0, 1))
    ...
    @Test
    fun `bookmarks round-trip newest-first and delete`() =
        runTest {
            val first =
                store.addBookmark(
                    Bookmark(bookId = "b1", chapterIndex = 0, passageIndex = 0, label = "a", createdAtEpochMillis = 10),
                )
            val second =
                store.addBookmark(
                    Bookmark(bookId = "b1", chapterIndex = 0, passageIndex = 1, label = "b", createdAtEpochMillis = 20),
                )
    ...
    @Test
    fun `migration backfills progress defaults and creates the player tables`() =
        runTest {
