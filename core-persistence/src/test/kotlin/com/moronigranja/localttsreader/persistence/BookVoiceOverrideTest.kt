...
class BookVoiceOverrideTest {
    ...
    @Test
    fun `set and clear round-trip through the generic table`() = runTest {
        ...
        assertEquals(
            ...
            database.settingsDao().all().filter { it.key.startsWith(SettingsStore.KEY_BOOK_VOICE_PREFIX) }
    ...
    @Test
    fun `an override changes only that book and never the global default`() = runTest {
        val store = SettingsStore(database.settingsDao())
        ...
        assertEquals("changing the global default neither clears nor rewrites overrides",
            "de_DE-thorsten-high", store.bookVoice("b1"))
    }

    @Test
    fun `blank values are no override`() = runTest {
        val store = SettingsStore(database.settingsDao())
    ...
    @Test
    fun `the override rides the backup archive and reattaches on restore`() = runTest {
        ...
        assertEquals("the override reattaches to the content-hash book id",
            "de_DE-thorsten-high", target.bookVoice("b1"))
    }
}
